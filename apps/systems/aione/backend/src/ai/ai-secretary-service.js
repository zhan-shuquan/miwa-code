...
export async function confirmAISecretaryProposal({ proposal, proposalId = null, officeCode, contextSnapshot, requestContext }) {
  ...
  try {
    const result = await pool.query("INSERT INTO public.work_items (id,title,work_type,status,priority,owner_person_id,goal_summary,description,platform_code,money_status,expected_result,due_at,source_system,created_by_person_id,updated_by_person_id) VALUES ($1,$2,'ai_assigned','pending',$3,$4,$5,$6,'aione','pending',$7,$8,'aione-ai-secretary',$4,$4) RETURNING *", [id,title,dbPriority,requestContext.personId,payload.reason || "AI秘书确认创建",payload.description || "",payload.reason || "待形成",payload.dueAt || null]);
    try { await pool.query("INSERT INTO public.business_events (id,event_type,object_type,object_id,actor_kind,actor_person_id,actor_ai_ref,happened_at,payload,source_system) VALUES ($1,'work.created','work_item',$2,'human',$3,'ai-secretary',NOW(),$4::jsonb,'aione-ai-secretary')", [makeId("evt"),id,requestContext.personId,JSON.stringify({ officeCode, confirmedByHuman:true, proposalId:resolvedProposalId })]); } catch (_) {}
