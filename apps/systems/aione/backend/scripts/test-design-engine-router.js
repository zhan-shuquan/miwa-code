...
const modelTask = {
  ...
  instruction_snapshot: {
    ...
    pageSpec: {
      presentation: { sceneStyle: "outdoor-work", framing: "lower-body", backgroundMode: "studio" },
...
const whiteTask = {
  ...
  instruction_snapshot: {
    pageType: "white_bg",
    preserveProductTruth: true,
    pageSpec: { presentation: { cleanupMode: "cleanup", shadowMode: "none" }, metadata: { sourceAnchored: true } }
  }
};
...
const specSvg = buildProductSpecSvg({
  ...
  facts: { supportedSize: "24–27cm", actualVariants: ["白色", "米色", "卡其", "军绿", "深灰", "黑色"], season: "秋冬", lengthType: "中筒", setCount: 6 }
});
...
const skuSvg = buildSkuColorSvg({
  facts: { actualVariants: ["白色", "米色", "卡其", "军绿", "深灰", "黑色"], setCount: 6 },
  sourceDataUri: fakeImage,
  pageSpec: { presentation: { titleText: "COLOR VARIATIONS" }, layoutAdjustments: { columns: 3 } }
});
...
const materialVisualOnly = buildMaterialTextureSvg({
  facts: {},
  sourceDataUri: fakeImage,
  pageSpec: { presentation: { copyMode: "confirmed-only", displayMode: "macro" } }
});
