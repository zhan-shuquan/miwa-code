...
for (const [name, mimeType] of cases) {
  ...
  assert.equal(first, second, "Rakuten provider filename must be deterministic");
  assert.match(first, /^[a-z0-9_-]+\.(jpg|png|gif)$/);
  assert.ok(Buffer.byteLength(first, "utf8") <= 20, `Rakuten provider filename exceeds 20 bytes: ${first}`);
}

assert.notEqual(
  buildRakutenSafeFilePath("主图 01.jpg", "image/jpeg"),
  buildRakutenSafeFilePath("主图 02.jpg", "image/jpeg"),
  "Different canonical names should not collapse to the same provider filename in the acceptance fixture"
);
