...
app.get("/health", async (req, res) => {
  try {
    ...
    res.json({ ok: true, service: "aione-backend", apiVersion: "v1", database: "connected", databaseTime: dbResult.rows[0].database_time, latestMigration: migrationResult.rows[0] || null });
  } catch (error) {
    console.error(error);
...
if (designCenterPreviewEnabled) {
  ...
  if (previewAccessToken) {
    app.use((req, res, next) => {
      ...
      if (!accepted) return res.status(401).type("text/plain").send("AIONE Design Center preview access required.");
      if (supplied === previewAccessToken && cookie !== previewAccessToken) res.append("Set-Cookie", `aione_preview_access=${encodeURIComponent(previewAccessToken)}; Path=/; Max-Age=21600; HttpOnly; Secure; SameSite=Lax`);
      next();
    });
  }
...
app.use((error, req, res, next) => {
  ...
  res.status(status).json({ error: error.code || (status === 500 ? "internal_error" : status === 401 ? "authenticated_actor_required" : status === 400 ? "bad_request" : "data_conflict"), message: status === 500 ? "AIONE backend request failed." : (error.message || "AIONE request failed."), missing: error.missing || undefined, remoteCode: error.remoteCode || undefined, detail: process.env.NODE_ENV === "production" ? undefined : error.message });
});

const port = Number(process.env.PORT || 8080);
app.listen(port, () => console.log(`AIONE Backend API listening on port ${port}`));
