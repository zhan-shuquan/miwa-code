...
must((config.includes('v1.9.7-miwa-ai-render-hotfix') || config.includes('v1.9.8-miwa-ai-entry-bridge-hotfix') || (config.includes('v1.9.9-miwa-ai-hard-entry-hotfix') || config.includes('v1.9.10-miwa-ai-self-healing-layer-hotfix') || config.includes('v1.9.11-miwa-ai-runtime-chain-verification')) || config.includes('v1.9.13-model-provider-layer') || (config.includes('v1.9.16-ai-proposal-bridge') || (config.includes('v1.9.17-ai-context-router') || (config.includes('v1.9.18-1688-source-api-bridge') || (config.includes('v1.9.19-1688-oauth-bridge') || config.includes('v1.9.20-1688-permanent-token-direct')))))), "系统版本未升级到V1.9.7");
...
must((layerJs.includes('const SUGGESTIONS') || layerJs.includes('routeAIONEAIContext')) && layerJs.includes('renderContext()'), "美和AI未按当前Route/Context生成上下文能力推荐");
...
must(baseline.includes('美和AI → AI工作区 → AI办公室') || baseline.includes('`美和AI → AI工作区 → AI办公室`'), "V1.9.7基线文档缺少AI升级路径");
must(baseline.includes('全局搜索 → 美和AI → 通知 → 帮助 → 设置') || baseline.includes('`全局搜索 → 美和AI → 通知 → 帮助 → 设置`'), "V1.9.7基线文档缺少Header顺序");

console.log("V1.9.7_MIWA_AI_LAYER_VALIDATION_OK");
