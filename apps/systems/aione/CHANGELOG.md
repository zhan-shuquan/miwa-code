## V1.9.39｜工作之家 V2.0 真实运行验证基线｜2026-08-28

- 工作之家从“最终锁定”调整为“真实运行验证基线”：结构稳定运行，但继续允许真实业务发现高价值能力。
- Sidebar更新为：工作概览、我的工作、我的关注、我的建议、我的创新、我的总结、全部工作、事业工作、团队工作、工作记录；创建工作退出Sidebar。
- Header工作之家默认进入“我的工作”；旧“今日工作”只保留兼容路由。
- 我的工作新增“今日以前 / 当前工作 / 今日以后”连续时间范围；工作跨天不自动消失。
- 我的工作、全部工作、事业工作、团队工作统一复用Work Item Browser；团队工作不再使用独立成员目录Main。
- Main保留“＋ AI创建工作”；核心指标升级为可点击筛选，保持单行并支持横向溢出。
- Work Item列表时间字段明确拆分为“开始时间 / 截止时间”。
- 工作记录升级为闭环工作事实账本，并增加真实工作总结状态、Google Docs/PDF关联兼容字段；没有真实summary metadata时不伪造总结。
- 新增“我的总结”个人视图；普通工作不强制总结，方向为本人主动发起、美和AI起草、人阅读判断与确认。
- 我的关注明确为“订阅 + 快速定位 + 持续跟踪”；当前仍只使用已存在的Work Item关注Backend，跨对象关注/收藏留待后续正式对象能力开发。
- 不新增数据库迁移，不修改Backend Schema；继续继承V1.9.36 Backend与0060数据库基线。

## V1.9.38｜工作之家 V2.0 最终收口｜2026-08-27

- 工作之家Sidebar收口：状态入口移入全部工作快速筛选，新增“＋ 创建工作”美和AI高频入口。
- 全部工作统一搜索/关系/事业/成员/状态/优先级/时间/更多筛选/排序/显示/刷新，并采用12项分页。
- 新增正式排序下拉：AI建议、优先级、截止时间、最近更新、最近创建。
- 当前事业统一回Header“事业之家｜当前事业▼”，业务Sidebar移除冗余事业切换。
- 修复工作概览 `businessHtml is not defined` 启动错误。
- 工作页采用“工”主题字占位；美和工作9问保持“验证中”。
- 不新增数据库迁移，不要求Backend重新部署；继续继承V1.9.36 Backend与0060数据库基线。

## V1.9.37｜工作之家结构收口｜2026-08-27

- Sidebar在“事业工作”之后新增“团队工作”：先按成员查看当前/本周/本月/本年工作分布，再进入同一Work Item Browser处理具体工作；团队工作用于协同与资源判断，不承担人员评价。
- 工作记录退出人员汇总主入口，收口为真实历史工作事实账本，只回看已完成/取消/归档工作以及对应结果、证据和闭环事实。
- 今日工作、全部工作、事业工作、团队工作明确不同使用意图并补充页面副标题；Header与移动端“工作之家”继续默认进入今日工作。
- “查看工作手册”升级为场景章节深链：从今日/全部/事业/团队/关注/等待/异常/验收/记录页面进入时，直接定位《美和工作手册》对应主标题，不再统一回封面。
- 工作事项显示控制收口为“列表｜卡片下拉｜刷新”；卡片下拉提供2/3/4/6列，保存用户最近列数与视图偏好，并按Main容器宽度自动降列保护可读性。
- 本版只做前端工作之家结构收口；不新增数据库迁移，不修改Backend Schema，继续复用V1.9.36点赞/关注、money_events、work_sessions及Proposal→执行→证据→验收→AI复盘闭环。

## V1.9.36｜工作之家统一浏览体验收口 + 点赞关注真实数据｜2026-08-27

- Work Item Browser统一搜索、高频筛选、一级时间筛选、更多筛选、重置、排序、卡片/列表、2/3/4列和刷新；刷新与视图工具固定为同一操作组，不再单独掉行。
- 工作事项卡片取消6列，默认3列，并根据Main真实容器宽度自动降为2列/1列，解决Sidebar+Aside存在时卡片被挤成窄条的问题。
- 全部工作新增全部时间/今日/本周/本月/本年/自定义；更多筛选新增时间依据、金额、最低金额，条件生效显示数量，“重置”恢复当前入口默认Query。
- 工作记录“工作明细”正式复用同一Work Item Browser；人员汇总继续使用人员卡片/列表，本周/月/年作为事实时间范围。
- work-home读取money_events形成财务摘要，涉及钱的工作在卡片/列表条件展示；读取work_sessions汇总已记录操作时间，不以页面停留时间冒充工作效率。
- 新增object_reactions迁移和点赞Backend：点赞/取消、点赞数、本人点赞状态、scope=liked；“我的点赞”只作为“我的关注”里的二级互动记录，不新增Sidebar。
- 关注继续复用work_item_participants(observer)，点赞与关注互不改变负责人、参与角色和执行权限。
- `/api/v1/work-home/capabilities`升级到V1.9.36，明确likes/timeFilter/moneySummary/workSessionSummary能力。

## V1.9.35｜工作之家统一工作事项一览 + 建议/创新集团归口｜2026-08-27

- Header“工作之家”默认路由改为今日工作；工作概览回归《美和工作手册》用途。
- Sidebar收敛为工作概览、今日工作、全部工作、我的关注、我的建议、我的创新、事业工作、等待中、异常处理、待验收、工作记录；常用事业优先显示，美和跨境/批发/留学 + 更多事业。
- 全部工作新增明显“今日工作”快捷入口；筛选采用高频常驻 + 更多筛选，新增“重置”恢复当前入口默认查询条件。
- 等待中、异常处理拆开：等待是正常外部等待，异常处理承接BLOCKED/错误信号；两者仍复用同一Work Item Browser。
- 工作记录人员汇总支持卡片/列表；工作明细改为直接复用Work Item卡片/列表；人员信息优先读取统一用户目录。
- 工作卡/列表预留点赞交互位置，但本版不写入假点赞数据；关注Backend不作为本轮发布阻塞。
- 新增工作之家“我的建议 / 我的创新”入口；美和之家 → 经营与战略新增建议中心、创新中心，明确个人提出 → 集团归口 → Work Item/Project → 验证 → 工作记录/人才贡献的闭环。
- 《美和工作手册》按最新Sidebar补齐建议、创新、事业、等待、异常、验收和工作记录指南。

## V1.9.33｜工作之家 V2.0 结构升级｜2026-08-27

- 工作之家从“个人任务页”扩展为集团级统一工作入口：正式Sidebar为工作概览、今日工作、全部工作、我的关注、事业工作、等待与阻塞、待验收、工作记录；`我的工作`保留兼容路由但不再占用正式Sidebar。
- 事业工作按美和跨境、美和批发、美和采购代理、美和物流、美和留学、美和不动产、美和商务咨询、美和品牌统一生成，事业是默认工作边界，仍可跨事业查看与关注。
- 新增统一Work Browser：卡片/列表双视图、搜索、关系/状态/优先级/事业筛选、真实团队/项目扩展筛选、AI建议优先/截止/最近更新/优先级排序。
- 新增关注机制：复用`work_item_participants`的`observer`角色，不新增重复关注表；关注项进入“我的关注”，但仅关注不会自动进入今日工作或Header待办，落实“透明 ≠ 推送”。
- `GET /api/v1/work-home`新增`scope=all/following/related`；公司内部公开工作可读取，`private/restricted/sensitive`继续受限；执行/提交/验收权限不因公开读取而扩大，落实“看得到 ≠ 要处理 ≠ 能操作”。
- 美和AI从跨境/批发业务Route创建新工作时写入`metadata.businessCode`与`metadata.visibility=company`，历史无事业信息工作不伪造归属。
- 工作概览从V1.9.32“出版物风格”升级为真正继承`miwa-publication-master`的《美和工作手册》：封面/封底、A4书页、双页/单页、章节、页码、Topline、Footer、打印/PDF全部统一，共8章。
- 完整继承V1.9.30执行/证据/提交/验收/AI复盘与V1.9.30.2 Header提醒同步；不新建第二套工作事实，不新建日历任务模型。

## V1.9.32｜工作之家 V2.0 第一次升级｜2026-08-27

- Sidebar升级为工作概览、今日工作、我的工作、全部工作、等待与阻塞、待验收、工作记录；状态下沉Main筛选。
- 工作概览升级为出版物式活手册，加入美和工作9问、人×AI分工与工作之家×美和日历关系。
- 今日工作加入第一版规则排序，工作卡执行减法。
- 完整继承V1.9.30执行证据、结果、人工验收、美和AI复盘与V1.9.30.2 Header提醒同步。
- 本版不修改数据库Schema、不开放全公司无边界读取、不创建第二套工作/日历任务。

## V1.9.31.5｜美和出版物内页与章节统一｜2026-08-27

- 美和之家与事业之家从“仅概览出版化”升级为“全部目录章节统一出版物排版”。
- 内页正式采用用户已确认的PPT式信息设计语言：大标题、少量说明、3/2栏编辑式内容块、大留白、结论条、页码与统一Footer。
- 一级目录继续作为章，二级目录作为节；章/节页面共用A4纵向双页书籍母版。
- 美和之家与事业之家非概览路由不再退回普通SaaS长页面，而是按内容类型生成书页。
- 左Sidebar下半区新增“打印 / 导出PDF”常规出版操作；右Aside保留同样入口，形成双入口。
- 打印/PDF仍保持A4纵向单页输出；桌面优先双页，空间不足自动单页。
- 本版完成后暂停内容型页面工程，开发主线转向美和跨境真实业务闭环。

## V1.9.31.4｜美和数字出版设计系统收口｜2026-08-27

- 美和之家、事业之家概览统一接入“美和数字出版设计系统 V1.0”。
- 默认A4纵向书页；宽屏优先双页展开，Main空间不足自动降级单页，避免机械压缩。
- 中等桌面优先保护书页可读性，Aside自动收起为紧凑打印/PDF操作；移动端保持单页连续阅读。
- 首屏采用“正封面 + 封底”双页陈列；正式打印/PDF时封底自动回到整册最后。
- 保留美和绿 #176B4D 与美和红 #C83A32 的出版识别；Main不放操作工具。
- 一级目录=章、二级目录=节、内容=页；奇数余页优先使用章总结/结束语。
- 本版完成后暂停内容页面工程，后续主线转向美和跨境真实业务闭环设计。

# V1.9.31.3｜内容出版双页书籍母版

- 内容型页面默认改为A4纵向书页；桌面端双页展开、移动端单页连续阅读、打印/PDF按真实A4纵向单页输出。
- 正式锁定：空间即书，一级目录即章，二级目录即节，内容即页，奇数余页优先用于章总结。
- 打印 / 导出PDF从Main移到Aside下半区；Main保持纯书页。
- 新增统一封面系统、页码、左右页书脊边距、章总结页、双页中缝视觉与出版页溢出检测。
- 事业之家概览作为第一套真实验证：封面单页，P2/P3、P4/P5、P6/P7双页展开；第01章用右页总结收口。
- 预留事业专属图标槽位；当前代码包未包含用户正式事业图标资产时，只使用语义图标作结构性回退，不冒充正式图标。

# V1.9.31.2｜AIONE内容展示型页面母版 V1.0

- 将事业之家“战略书页”提升为平台级可复用内容母版，正式区分内容展示型Main与业务执行型Main。
- Sidebar继续使用平台固定232px，不因内容页改变；内容页Aside调整为light 260px / standard 292px，并适当收紧Main外部padding。
- 新增通用 `miwa-publication-master.css/js`，统一A4横向书页、出版工具栏、PDF/打印、移动端滚动与路由模式恢复。
- 事业之家概览新增 `打印 / 导出PDF` 按钮；打印时自动隐藏Global Shell，只保留A4横向书页。
- 内容书页视觉颜色收敛为美和绿 + 美和红，移除蓝/金/棕色常规强调线。
- 移动端恢复并强化纵向滚动；能保持书页比例时保持，内容超出时允许自然增长，不裁切正文。
- 新增正式规范《AIONE内容展示型页面母版 V1.0》，后续美和之家、人才之家、AI之家、客户之家等内容页优先复用。

# V1.9.31｜事业之家数字出版型页面

- 正式建立事业之家两级Sidebar与内容边界。
- 事业首页按“战略内容型页面母版”呈现集团事业版图。
- 纳入美和采购代理：已有真实业务基础，当前只做事业介绍，独立客户系统留待AIONE稳定后另立项目。
- 仅美和跨境、美和批发显示“进入事业”；未开发事业不冒充系统已完成。
- 美和AI接入事业之家正式上下文和事业专属快捷能力。
- 内容型页面支持A4打印/导出，并参考《美和集团AI执行总架构 V0.1》PPT的信息表达语言。
...
## V1.9.30.2｜工作之家全局提醒实时同步｜2026-08-26
- Header「工作之家」红色数字语义收敛为“当前需要我处理的工作”，不再显示全部未完成工作总数。
- 提醒纳入：本人待处理、需要本人确认、本人超期、阻断/异常；普通进行中与已完成不进入红色提醒。
- 新增全局 Work Attention Sync：Proposal确认、工作状态变化后立即刷新。
