...
function enhanceDailyAwareness() {
  ...
  const chineseZodiac = appendDailyItem(daily, "miwaChineseZodiac", "miwa-daily-zodiac");
  chineseZodiac.textContent = getChineseZodiacLabel(dateParts.year);
  const starSign = appendDailyItem(daily, "miwaStarSign", "miwa-daily-star-sign");
  starSign.textContent = getStarSignLabel(dateParts.month, dateParts.day);

  const term = appendDailyItem(daily, "miwaSolarTerm", "miwa-daily-term");
