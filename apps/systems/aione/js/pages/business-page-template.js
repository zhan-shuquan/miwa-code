...
function fieldValue(field, object) {
  ...
  if (field.format === "money") return money(value);
  if (field.type === "url") return value ? "已设置" : "待确认";
...
function mapImportedRows(rows, definition) {
  const labels = new Map(definition.fields.map((field)=>[field.label,field.key]));
  return rows.map((row)=>{const result={};Object.entries(row||{}).forEach(([key,value])=>{const target=labels.get(key)||(definition.fields.some((field)=>field.key===key)?key:null);if(target)result[target]=value;});return result;}).filter((row)=>Object.keys(row).length);
}
...
export async function initBusinessPage() {
  ...
  function renderTypes(){if(!typeBlock)return;const rows=normalizedTypeCards(definition,objects);typeBlock.body.dataset.visible=String(Math.min(3,Math.max(1,rows.length||1)));typeBlock.body.innerHTML=rows.map((type)=>`<article class="miwa-business-type-card${type.priority?' is-priority':''}${workspace.getState().filter===type.label?' is-active':''}"><button type="button" class="miwa-business-type-card__summary" data-business-type="${esc(type.label)}"><span><strong>${esc(type.label)}</strong><small>${esc(type.description)}</small></span><b>${type.count}</b></button><div class="miwa-business-type-card__actions"><button type="button" data-business-create-type="${esc(type.label)}" ${params.permissions?.allowCreate===false?'disabled':''}>＋ ${esc(definition.createLabel||`新建${definition.objectName}`)}</button><button type="button" data-business-type="${esc(type.label)}">查看全部</button></div></article>`).join("");}
  ...
  if(params.ai?.pageAssistantEnabled!==false)window.dispatchEvent(new CustomEvent("aione:page-ai-context",{detail:clone(definition.ai||{})}));else window.dispatchEvent(new CustomEvent("aione:page-ai-context",{detail:{title:"上下文辅助",text:"页面级AI秘书已在系统参数中关闭；当前仅保留必要业务提示。"}}));
