...
function pageHeader(){
  return `<header class="brand-page-header">
    <div><span>商品之家 · 第02章</span><h1>品牌中心</h1><p>统一回答“商品是谁的”：一个品牌一个 Brand 对象，品牌事实、权利和渠道映射共用同一事实源。</p></div>
  ...
  return `<div class="brand-view">
    ...
    <section class="brand-workspace">
      ...
      <aside class="brand-context-panel"><div class="brand-context-head tone-${brand.tone}"><span>${brand.mark}</span><div><small>${brand.type}</small><h2>${brand.name}</h2><p>${brand.kana}</p></div></div>
        ...
        <div class="brand-context-section"><h3>当前需要关注</h3><button data-brand-ai-prompt="检查 ${brand.name} 的品牌事实、品牌权利资料和渠道映射缺口；缺少真实证据的内容标记待确认。">✦ AI检查品牌完整度</button><button data-brand-view-link="rights">查看品牌权利 →</button><button data-brand-view-link="mapping">查看渠道映射 →</button></div>
      </aside>
    </section>
  </div>`;
}
...
function rightsView(){
  ...
  return `<section class="brand-rights">
    <header class="brand-section-head"><div><span>Brand Rights</span><h2>品牌权利</h2><p>只管理与品牌直接相关的商标、品牌所有权、授权关系、期限与权利证据；其他知识产权不在本章展开。</p></div><div><button class="brand-btn" data-brand-action="ai-rights">✦ AI申请前检查</button><button class="brand-btn brand-btn--primary" data-brand-action="trademark">＋ 申请商标</button></div></header>
    <div class="brand-rights-layout">
      ...
      <div class="rights-current"><span>当前品牌</span><h3>${brand.name}</h3><div class="rights-kpis"><div><small>品牌类型</small><strong>${brand.type}</strong></div><div><small>权利状态</small><strong>${brand.rights}</strong></div></div><div class="rights-ai"><b>商品之家AI秘书</b><p>先检查申请主体、文字 / 图形商标、指定商品候选、近似检索准备和资料缺口；正式法律判断与正式提交保留人工确认。</p><button data-brand-ai-prompt="为 ${brand.name} 做商标申请前检查：区分已知事实、待确认资料、建议准备顺序，不要编造法律结论。">开始AI检查</button></div></div>
    </div>
  </section>`;
}
...
function bind(){
  ...
  host.addEventListener('click',(event)=>{
    ...
    if(action==='trademark') openAI(`我要为 ${currentBrand().name} 准备商标申请。请按申请主体、商标形式、国家地区、指定商品或服务、近似检索、资料准备、人工确认、官方提交的顺序辅助我。`);
