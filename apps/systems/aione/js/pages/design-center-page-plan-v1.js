const PAGE_TYPES = [
  ...
  { code:"detail", name:"细节 / 结构图", required:false, output:"1000×1500", status:"ready", fields:["已确认结构卖点","局部说明"], assets:["细节图","产品图"], note:"已接入真实细节素材、细节类型、布局微调和设计任务；AI 只允许裁切、清理、排版，不发明商品结构。" }
];
...
function renderPagePlanCard(type) {
  const actions = {
    ...
    detail:'<button type="button" class="dc-button dc-button--primary" data-action="jump-detail">进入设计</button>'
  };
...
function createSection() {
  ...
  section.innerHTML = `<div class="dc-page-plan__intro"><div><h2>图片设计规划</h2><p>这件商品的 8 类通用图片角色已经全部接入同一套 Page Spec / DesignTask 底层，后续分类只需决定启用哪些模板和字段。</p></div><div class="dc-page-plan__legend"><span>字段来自 Product Truth</span><span>素材来自 01 / 02 / 03</span><span>设计中心只管呈现</span></div></div><div class="dc-page-plan-grid" data-ui="page-plan-grid"></div>`;
  return section;
}
...
export function initDesignCenterPagePlan() {
  ...
  import("./design-center-sku-v1.js?v=20260913-v2").then(async ({ initDesignCenterSkuV1 }) => {
    ...
    section.querySelector('[data-action="jump-sku"]')?.addEventListener("click", () => root.querySelector('[data-section="sku-design"]')?.scrollIntoView({ behavior:"smooth", block:"start" }));
    ...
    const { initDesignCenterMaterialV1 } = await import("./design-center-material-v1.js?v=20260913-v1");
    await initDesignCenterMaterialV1();
    section.querySelector('[data-action="jump-material"]')?.addEventListener("click", () => root.querySelector('[data-section="material-design"]')?.scrollIntoView({ behavior:"smooth", block:"start" }));
    ...
    const { initDesignCenterDetailV1 } = await import("./design-center-detail-v1.js?v=20260913-v1");
    await initDesignCenterDetailV1();
    section.querySelector('[data-action="jump-detail"]')?.addEventListener("click", () => root.querySelector('[data-section="detail-design"]')?.scrollIntoView({ behavior:"smooth", block:"start" }));
  });
}
