/* ========================================
   ...
   Legacy Business Space / Workbench sidebar routing has been removed.
======================================== */
...
function buildWorkItems() {
  ...
  const businessChildren = preferredBusinessIds
    ...
    .filter(Boolean)
    .map((item) => ({
      id: `work-business-${item.id}`,
      label: item.name,
      route: `work-business-${item.id}`,
      subtitle: `按${item.name}快速查看工作进展。`
    }));

  businessChildren.push({
    id: "work-business-more",
    label: "更多事业",
    route: "work-business-more",
    subtitle: "查看其他事业工作，仍然读取同一份Work Item。"
  });

  return [
    { id:"work-home", label:"工作概览", route:"work", icon:"work", children:[] },
    { id:"work-mine", label:"我的工作", route:"work-mine", icon:"work", children:[] },
...
function buildRegistryHomeItems(rootId) {
  ...
  if (definition.overview?.enabled) {
    items.push({
      ...
      label: "概览",
      route: rootId,
      icon: PLATFORM_CONTEXT_META[rootId]?.icon || "apps",
      children: []
    });
  }

  definition.centers.forEach((item) => {
    items.push({
      id: item.id,
      label: item.label,
      ...
      icon: item.icon || "",
      children: [],
      sectionGapBefore: Boolean(item.sectionGapBefore)
    });
  });

  if (definition.management?.enabled) {
    items.push({
      id: `${rootId}-management`,
      label: "管理",
      route: `${rootId}?view=management`,
      icon: "settings",
      children: [],
      sectionGapBefore: true
    });
  }
...
function buildPlatformItems(rootId) {
  ...
  const homeLabel = children.length
    ? "概览"
    : (root?.label || HOME_REGISTRY[rootId]?.label || "概览");
