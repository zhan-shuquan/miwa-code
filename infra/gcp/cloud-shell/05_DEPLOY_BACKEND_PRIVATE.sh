...
gcloud run deploy "$RUN_SERVICE" \
  ...
  --update-env-vars="DB_USER=${DB_USER},DB_NAME=${DB_NAME},INSTANCE_UNIX_SOCKET=${INSTANCE_UNIX_SOCKET},NODE_ENV=production,AIONE_ALLOW_PREVIEW_ACTOR=false,AIONE_ALLOW_SYSTEM_WRITES=false,AIONE_REQUIRE_GOOGLE_AUTH=true,AIONE_GOOGLE_CLIENT_ID=${GOOGLE_CLIENT_ID},AIONE_AI_MODE=${AI_MODE},AIONE_CORS_ORIGINS=https://aione.miwa-happyhouse.com\,http://127.0.0.1:5500\,http://localhost:5500" \
