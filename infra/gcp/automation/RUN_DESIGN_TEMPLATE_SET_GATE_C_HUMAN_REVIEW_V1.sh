...
[[ "$CURRENT_BRANCH" == "$BRANCH" ]] || {
  echo "[AIONE][STOP] Gate C human review must run from ${BRANCH}; current=${CURRENT_BRANCH}" >&2
  exit 20
}
[[ -z "$(git status --porcelain)" ]] || {
  echo '[AIONE][STOP] Repo must be clean before Gate C human review.' >&2
  exit 21
}
...
JOB="aione-design-gate-c-human-review"
...
printf '\n[AIONE] Gate C Explicit Human Review V1\n'
printf 'Branch       : %s\n' "$BRANCH"
printf 'Branch SHA   : %s\n' "$SHORT_SHA"
printf 'Asset ID     : %s\n' "$ASSET_ID"
printf 'Outcome      : %s\n' "$OUTCOME"
...
set -e
printf '%s\n' "$OUTPUT"
EXECUTION="$(printf '%s\n' "$OUTPUT" | grep -Eo 'aione-design-gate-c-human-review-[a-z0-9]+' | tail -n1 || true)"
...
printf '\n[AIONE] GATE C HUMAN REVIEW PASS\n'
printf 'Asset ID      : %s\n' "$ASSET_ID"
