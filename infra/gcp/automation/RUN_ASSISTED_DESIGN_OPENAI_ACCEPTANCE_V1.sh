...
echo '[AIONE] 3/4 Deploy one ephemeral CURRENT OpenAI acceptance job'
...
[[ -n "$EXECUTION" ]] || { echo '[AIONE][STOP] OpenAI acceptance execution id missing.' >&2; exit 26; }
...
[[ "$CODE" -eq 0 ]] || { echo '[AIONE][STOP] OpenAI image acceptance failed. Root-cause logs are printed above.' >&2; exit 27; }
...
grep -q '"provider": "openai"' <<<"$LOGS" || { echo '[AIONE][STOP] OpenAI provider evidence missing.' >&2; exit 30; }
grep -q '"canvas": "1000x1500"' <<<"$LOGS" || { echo '[AIONE][STOP] 1000x1500 output evidence missing.' >&2; exit 31; }
grep -q '"reviewStatus": "pending"' <<<"$LOGS" || { echo '[AIONE][STOP] Output must remain pending explicit human visual review.' >&2; exit 32; }
grep -q '"gcsObject":' <<<"$LOGS" || { echo '[AIONE][STOP] DERIVED GCS provenance missing.' >&2; exit 33; }
